...
private:
    ...
    __aicore__ inline void dumpWindowAbi()
    {
        ...
        AscendC::printf("A5 e2a[rank=%d] tree: localUsrRankId=%u rankSize=%u winSize=%llu "
        ...
        for (int i = 0; i < this->rankSize && i < 16; i++) {
            ...
            AscendC::printf("A5 e2a[rank=%d] tree: remoteRes[%d].nextDevicePtr=%llx windowsIn=%llx\n",
        ...
        for (int i = 0; i < this->rankSize && i < 16; i++) {
            AscendC::printf("A5 e2a[rank=%d] flat: windowsIn[%d]=%llx windowsOut[%d]=%llx\n",
