...
## DSV4 Flash async CAM concurrent requests (local, 16 or 8 NPUs)

`afd-dsv4-flash-async-cam-dp2tp4-ep8` runs Attention DP2/TP4 on the first
eight devices and FFN DP8/TP1/EP8 on the last eight.
`afd-dsv4-flash-async-cam-dp2tp2-ep4` runs the same deployment on eight
devices: Attention DP2/TP2 on the first four and FFN DP4/TP1/EP4 on the last
four. Both are standalone Ascend 910C cases, outside the four-device PR gate.
Use DSV4 Flash W8A8 weights and a DSV4-capable vLLM/vLLM-Ascend runtime with
CAM operators.
...
The DSV4 cases allow 60 seconds for service shutdown before escalation;
...
# 16 NPUs: Attention DP2/TP4 + FFN DP8/TP1/EP8
...
# 8 NPUs: Attention DP2/TP2 + FFN DP4/TP1/EP4
export AFD_E2E_DEVICES=0,1,2,3,4,5,6,7
python -m pytest -q -s \
  'tests/e2e/models/deepseek_v4_flash/test_async_cam_npu.py::test_deepseek_v4_flash_async_cam[afd-dsv4-flash-async-cam-dp2tp2-ep4]'
```
...
`/usr/local/Ascend/cann-9.0.1/opp/vendors/CAM`. Model and the exact device
list of the selected case must be supplied explicitly; missing setup fails
rather than skips.
